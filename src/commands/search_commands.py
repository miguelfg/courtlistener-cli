...
@click.option("--type", "search_type", default="r", help="Type code (default: r).")
@click.option(
    "--court",
    default=None,
...
def search_query(
    ...
    try:
        ...
        effective_stem = (
            f"{filename_stem}_partial_{returned}" if is_partial else filename_stem
        )

        if "results" in output_data:
            if output_format == "json":
                filepath = save_json(
                    output_data, output_dir, filename_stem=effective_stem
                )
            elif output_format == "csv":
                filepath = save_csv(
                    output_data["results"],
                    output_dir,
                    filename_stem=effective_stem,
                    concat_list_fields_char=sep,
                )
            else:  # xlsx
                filepath = save_xlsx(
                    output_data["results"],
            ...
            if is_partial:
                click.echo(
                    f"⚠ Partial export — daily quota hit after {output_data.get('pages_fetched', 0)} page(s)"
                )
            ...
            if slim:
                ...
                if output_format == "json":
                    slim_path = save_json(
                        slim_payload, output_dir, filename_stem=slim_stem
                    )
                elif output_format == "csv":
                    slim_path = save_csv(
                        slim_data,
