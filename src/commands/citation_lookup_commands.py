...
def lookup_text(text, output_format, output_path):
    ...
    try:
        ...
        click.echo(f"  Found: {len(found)}, Not found: {len(not_found)}, "
...
def lookup_citation(volume, reporter, page, output_path):
    ...
    try:
        result = client.request('POST', '/citation-lookup/',
        ...
        if results:
            ...
            if status == 200:
                ...
                click.echo(f"✗ Not found: {first.get('citation')} — {first.get('error_message')}")
            elif status == 300:
                click.echo(f"⚠ Ambiguous: {first.get('citation')} — "
