...
@click.option(
    "--confirm", is_flag=True, required=True, help="Confirm deletion (required)"
)
    ...
    try:
        ...
        if "results" in output_data:
            if output_format == "json":
                ...
                filepath = save_xlsx(output_data["results"], output_dir)
            click.echo(
                f"✓ Exported {output_data.get('returned_count', 0)} docket alerts"
            )
            click.echo(f"✓ Saved to {filepath}")
        else:
...
def create_docket_alert(docket, alert_type):
    ...
    try:
        result = client.request(
            "POST", "/docket-alerts/", json={"docket": docket, "alert_type": alert_type}
        )
        click.echo(json.dumps(result, indent=2))
...
@click.option(
    "--id", "alert_id", required=True, type=int, help="Docket alert ID to delete"
)
@click.option(
    "--confirm", is_flag=True, required=True, help="Confirm deletion (required)"
)
