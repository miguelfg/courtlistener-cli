...
@click.option(
    "--docket-entry", default=None, type=int, help="Filter by docket entry ID"
)
@click.option(
    "--is-available",
    default=None,
...
def list_recap_documents(
    docket_entry, is_available, limit, max_pages, order_by, output_format, output_path
):
...
def query_recap(court, pacer_doc_id, output_format, output_path):
    ...
    try:
        ...
        if output_format == "json":
            filepath = save_json(
                {"results": results, "count": len(results)}, output_dir
            )
        elif output_format == "csv":
