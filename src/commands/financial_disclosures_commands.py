...
def _list_endpoint(endpoint: str, resource_label: str, extra_params: dict,
                   limit: int, max_pages: int, output_format: str, output_path: str):
    ...
    try:
        ...
        if 'results' in output_data:
            ...
            click.echo(f"✓ Exported {output_data.get('returned_count', 0)} {resource_label}")
...
def list_disclosures(person, year, limit, max_pages, output_format, output_path):
    ...
    if year is not None:
        extra['year'] = year
    _list_endpoint('/financial-disclosures/', 'disclosures', extra,
                   limit, max_pages, output_format, output_path)
...
@click.option('--disclosure', default=None, type=int,
              help='Filter by financial disclosure ID')
@click.option('--redacted', default=None, type=bool,
              help='Filter to rows with redacted data (True/False)')
@click.option('--gross-value-code', default=None,
...
@click.option('--format', 'output_format', default='json',
              type=click.Choice(['json', 'csv', 'xlsx']))
@click.option('--output', 'output_path', default='./output', type=click.Path())
def list_investments(disclosure, redacted, gross_value_code,
                     limit, max_pages, output_format, output_path):
    ...
    _list_endpoint('/investments/', 'investments', extra,
                   limit, max_pages, output_format, output_path)
...
@click.option('--disclosure', default=None, type=int,
              help='Filter by financial disclosure ID')
@click.option('--redacted', default=None, type=bool,
              help='Filter to rows with redacted data')
@click.option('--limit', default=20, type=int)
@click.option('--max-pages', default=10, type=int)
@click.option('--format', 'output_format', default='json',
              type=click.Choice(['json', 'csv', 'xlsx']))
...
def list_gifts(disclosure, redacted, limit, max_pages, output_format, output_path):
    ...
    if redacted is not None:
        extra['redacted'] = redacted
    _list_endpoint('/gifts/', 'gifts', extra, limit, max_pages, output_format, output_path)
...
def list_debts(disclosure, redacted, limit, max_pages, output_format, output_path):
    ...
    if redacted is not None:
        extra['redacted'] = redacted
    _list_endpoint('/debts/', 'debts', extra, limit, max_pages, output_format, output_path)
...
def list_agreements(disclosure, limit, max_pages, output_format, output_path):
    ...
    if disclosure is not None:
        extra['financial_disclosure'] = disclosure
    _list_endpoint('/agreements/', 'agreements', extra,
                   limit, max_pages, output_format, output_path)
...
@click.option('--format', 'output_format', default='json',
              type=click.Choice(['json', 'csv', 'xlsx']))
@click.option('--output', 'output_path', default='./output', type=click.Path())
def list_non_investment_incomes(disclosure, limit, max_pages, output_format, output_path):
    """List non-investment earned income (≥$200) in financial disclosures"""
    extra = {}
    if disclosure is not None:
        extra['financial_disclosure'] = disclosure
    _list_endpoint('/non-investment-incomes/', 'non-investment-incomes', extra,
                   limit, max_pages, output_format, output_path)
...
def list_disclosure_positions(disclosure, limit, max_pages, output_format, output_path):
    ...
    if disclosure is not None:
        extra['financial_disclosure'] = disclosure
    _list_endpoint('/disclosure-positions/', 'disclosure-positions', extra,
                   limit, max_pages, output_format, output_path)
